...
def load_portfolio_from_excel(config_path="config.yaml") -> pd.DataFrame:
    ...
    def extract_named_range_values(wb, sheet, range_name):
        try:
            ...
            raise KeyError(f"❌ Named range '{range_name}' not found in workbook.")

        destinations = list(dn.destinations)
        if not destinations:
            raise ValueError(f"❌ No destinations found for named range '{range_name}'.")
        ...
        raise ValueError(f"❌ Named range '{range_name}' not defined on sheet '{sheet.title}'.")
    ...
    if df.empty:
        raise ValueError("❌ DataFrame is empty after parsing and cleaning. Check Excel content.")
    ...
    if not abs(total_weight - 1.0) < 1e-6:
        raise ValueError(f"❌ Portfolio weights sum to {total_weight:.4f}, expected 1.0.")
